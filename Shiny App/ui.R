...
selectInput_tooltip_container <- function(inputId, label, choices, tooltipText, selected) {
  tags$div(
    class = "input-container",
    tags$div(
      class = "input-tooltip-container",
    ...
    selectInput(inputId, NULL, choices, selected=selected)
  )
}
...
ui <- fluidPage(
    ...
    fluidRow(
      column(
        ...
        p(HTML("This tool accompanies the study <a href='https://www.nature.com/articles/s41560-024-01649-w'> 'U.S. Industrial Policy Could 
               ...
               <a href='https://www.nature.com/nenergy/'><i>Nature Energy</i></a>, which was published Oct. 2 2024, reflecting the 
               regulatory interpretation of the Inflation Reduction Act (IRA) at the time. Each pre-defined scenario approximates the 
               projected incentives received by that car model. The default scenario represents an approximation of the current policy 
               landscape. This tool was last updated Oct. 6 2025.")),
        tags$ul(
          ...
          tags$li(HTML("Costs were modeled with the Argonne National Lab (ANL)'s <a href='https://www.anl.gov/cse/batpac-model-software'>BatPaC</a> 
                            model, using critical minerals data from the ANL and Department of Energy's 
      ...
      column(
        ...
        img(
          src = "cmu_logo.png",
          width = "90%",  # scales image to column width
  ...
  navset_tab(
    nav_panel(title = "Credit Scenario Explorer",
              ...
              sidebarLayout(
                sidebarPanel(
                  ...
                  selectInput_tooltip_container("USScen", "US Pre-defined Scenarios", 
                                                choices = c("User","2024 Tesla Model Y Long Range (75 kWh)", 
                                                            ...
                                                            "Post-OBBBA Tesla Model Y vs Chinese NCA generic"),
                                         ...
                                         production costs, supply chain choices, or credit qualifications. Note that 
                                         Tesla only uses NCA in some long-range models (and not even all 2024 Model Ys);
                                         all standard range Teslas in China are made with the LFP chemistry.",
                                                "Post-OBBBA Tesla Model Y vs Chinese NCA generic"),
                  # Checkboxes
                  input_switch_tooltip('45XCM', '45X Critical Mineral Processing Credit', value = TRUE, "A 45X Manufacturing Credit. Currently defined as 10% of 
                                production cost for mineral extraction only (not the value of the minerals themselves). Claimable for domestic 
                                (US-based) production only."),
                  input_switch_tooltip('45XEAM', '45X Electroactive Materials Production Credit', value = TRUE, "A 45X Manufacturing Credit. Currently defined 
                                as 10% of production cost for production of electroactive materials (not the value the materials), e.g. cathodes, 
                                anodes, electrolyte. Claimable for domestic (US-based) production only."),
                  input_switch_tooltip('45XCell', '45X Battery Cell Production Credit', value = TRUE, "A 45X Manufacturing Credit. $35 per kilowatt-
                                hour of battery cells produced. Claimable for domestic (US-based) production only."),
                  input_switch_tooltip('45XMod', '45X Battery Module Production Credit', value = TRUE, "A 45X Manufacturing Credit. $10 per kilowatt-hour
                  ...
                  input_switch_tooltip('30DCrit', '30D Critical Minerals-Based Purchase Credit', value = TRUE, "A 30D New Clean Vehicle Credit. $3750 per vehicle, 
                  ...
                  input_switch_tooltip('30DComp', '30D Battery Component-Based Purchase Credit)', value = TRUE, "A 30D New Clean Vehicle Credit. $3750 per vehicle, 
                  ...
                  input_switch_tooltip('45WLease', '45W Credit for commercial vehicles (including leasing)', value = FALSE, "The 45W Commercial Clean 
                  ...
                  input_slider_tooltip('CM_slider', 'Critical Mineral Processing Credit slider (%)', min = 0, max = 100, 
                                       value = 10, step = 1, round = 0, tooltipText = "The IRA currently allows material processing companies to claim 10% of the
                                production cost for mineral extraction, but recently introduced legislation proposes increasing this to upwards of 25%."),
                  
                  input_slider_tooltip('EAM_slider', 'Electroactive Materials Production Credit slider (%)', min = 0, max = 100, 
                                       value = 10, step = 1, round = 0, tooltipText = "The IRA currently allows material processing companies to claim 10% of the
                                production cost for electroactive materials. It is unclear if recently introduced legislation will change this."),
                ...
                mainPanel(
                  ...
                  card(
                    height = 100,
                    card_header("Lithium-ion battery chemistry choices to show"),
                    card_body(
                      min_height = 50,
                      layout_column_wrap(
                        width = 1/4,
                  ...
                  card(
                    height = 120,
                    card_header("Costs and incentives to show"),
                    card_body(
                      min_height = 50,
                      layout_column_wrap(
                        width = 1/4,
                        input_switch("graphUSCost", "USA Battery Production Costs", value = TRUE),
                        input_switch("graphUSInc", "USA Federal EV Incentives", value = TRUE),
                        input_switch("graphChCost", "China Battery Production Costs", value = TRUE),
                        input_switch("graphChInc", "China National EV Incentives", value = TRUE),
                      )
                    ),
                  ),
    ...
    nav_menu(
      ...
      nav_item(tags$a("Link to paper", href = "https://www.nature.com/articles/s41560-024-01649-w"))
    )
  ),
